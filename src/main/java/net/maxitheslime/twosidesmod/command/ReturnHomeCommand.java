package net.maxitheslime.twosidesmod.command;
...
public class ReturnHomeCommand {
    ...
    private int execute(CommandContext<CommandSourceStack> context) {
        ...
        boolean hasHomepos = player.getPersistentData().getIntArray("twosidesmod.homepos").length != 0;

        if(hasHomepos) {
            int[] playerPos = player.getPersistentData().getIntArray("twosidesmod.homepos");

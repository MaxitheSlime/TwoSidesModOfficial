...
public class WaterArmorItem extends ArmorItem implements GeoItem
{
    private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);

    private static final Map<ArmorMaterial, MobEffectInstance> WATER_ARMOR_EFFECT1 =
            (new ImmutableMap.Builder<ArmorMaterial, MobEffectInstance>())
                    .put(ModArmorMaterials.CONTROL_WATER, new MobEffectInstance(ModEffects.CONDUIT_BLESSING.get(), 200, 0))
                    .build();
    private static final Map<ArmorMaterial, MobEffectInstance> WATER_ARMOR_EFFECT2 =
            (new ImmutableMap.Builder<ArmorMaterial, MobEffectInstance>())
                    .put(ModArmorMaterials.CONTROL_WATER, new MobEffectInstance(MobEffects.DIG_SLOWDOWN, 200, 0))
                    .build();

    public WaterArmorItem(ArmorMaterial armorMaterial, Type type, Properties properties) {
        ...
        consumer.accept(new IClientItemExtensions() {
            ...
            public @NotNull HumanoidModel<?> getHumanoidArmorModel(LivingEntity livingEntity, ItemStack itemStack, EquipmentSlot equipmentSlot, HumanoidModel<?> original) {
                if (this.renderer == null)
                    this.renderer = new WaterArmorRenderer();
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, 20, state -> {
            ...
            boolean isFullSet = wornArmor.containsAll(ObjectArrayList.of(
                    ModItems.WATER_BOOTS.get(),
                    ModItems.WATER_LEGGINGS.get(),
                    ModItems.WATER_CHESTPLATE.get(),
                    ModItems.WATER_HELMET.get()));
    ...
    private void evaluateArmorEffects(Player player) {
        for(Map.Entry<ArmorMaterial, MobEffectInstance> entry : WATER_ARMOR_EFFECT1.entrySet()) {
        ...
        for(Map.Entry<ArmorMaterial, MobEffectInstance> entry : WATER_ARMOR_EFFECT2.entrySet()) {
            ArmorMaterial mapArmorMaterial = entry.getKey();
            MobEffectInstance mapEffect = entry.getValue();

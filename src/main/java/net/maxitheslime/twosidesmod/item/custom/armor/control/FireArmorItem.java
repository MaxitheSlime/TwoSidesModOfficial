...
public class FireArmorItem extends ArmorItem implements GeoItem
{
    private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);

    private static final Map<ArmorMaterial, MobEffectInstance> FIRE_ARMOR_EFFECT1 =
            (new ImmutableMap.Builder<ArmorMaterial, MobEffectInstance>())
                    .put(ModArmorMaterials.CONTROL_FIRE, new MobEffectInstance(ModEffects.FIRE_FISTS.get(), 200, 0))
                    .build();
    private static final Map<ArmorMaterial, MobEffectInstance> FIRE_ARMOR_EFFECT2 =
            (new ImmutableMap.Builder<ArmorMaterial, MobEffectInstance>())
                    .put(ModArmorMaterials.CONTROL_FIRE, new MobEffectInstance(MobEffects.FIRE_RESISTANCE, 200, 0))
                    .build();

    public FireArmorItem(ArmorMaterial armorMaterial, Type type, Properties properties) {
        ...
        consumer.accept(new IClientItemExtensions() {
            ...
            public @NotNull HumanoidModel<?> getHumanoidArmorModel(LivingEntity livingEntity, ItemStack itemStack, EquipmentSlot equipmentSlot, HumanoidModel<?> original) {
                if (this.renderer == null)
                    this.renderer = new FireArmorRenderer();
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, 20, state -> {
            ...
            boolean isFullSet = wornArmor.containsAll(ObjectArrayList.of(
                    ModItems.FIRE_BOOTS.get(),
                    ModItems.FIRE_LEGGINGS.get(),
                    ModItems.FIRE_CHESTPLATE.get(),
                    ModItems.FIRE_HELMET.get()));
    ...
    private void evaluateArmorEffects(Player player) {
        for(Map.Entry<ArmorMaterial, MobEffectInstance> entry : FIRE_ARMOR_EFFECT1.entrySet()) {
        ...
        for(Map.Entry<ArmorMaterial, MobEffectInstance> entry : FIRE_ARMOR_EFFECT2.entrySet()) {
            ArmorMaterial mapArmorMaterial = entry.getKey();
            MobEffectInstance mapEffect = entry.getValue();
